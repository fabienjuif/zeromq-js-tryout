...
const addClient = (identity, topic) => {
  if (!clients.has(identity)) clients.insert(identity, { identity, topics: new Set() })
...
const addTopic = (name, worker) => {
  if (!topics.has(name)) topics.insert(name, { name, workers: new Set(), lastWorkerIndex: 0 })

  const topic = topics.get(name)
  topic.workers.add(worker)

  return topic
}

const addWorker = (identity, topicName) => {
  const client = addClient(identity, [topicName])
  const topic = addTopic(topicName, identity)
